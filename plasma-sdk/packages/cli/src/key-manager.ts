...
export class KeyManager {
  ...
  private async initKeytar(): Promise<typeof import('keytar') | null> {
    ...
    try {
      ...
      console.warn('OS keyring not available, falling back to encrypted file storage');
      return null;
    }
  }
   ...
   */
  async generateWallet(name?: string): Promise<{ address: Address; privateKey: Hex }> {
   ...
   */
  validatePrivateKey(privateKey: string): { valid: boolean; address?: Address; error?: string } {
    try {
    ...
    if (this.useKeyring) {
      ...
      if (keytar) {
        try {
          ...
          console.warn('Failed to store in keyring, falling back to encrypted file');
        }
      }
    }
  ...
  async retrieveKey(
    ...
    if (envKey) {
      ...
      if (validation.valid && validation.address?.toLowerCase() === address.toLowerCase()) {
   ...
   */
  async mnemonicToPrivateKey(mnemonic: string, path: string = "m/44'/60'/0'/0/0"): Promise<Hex> {
