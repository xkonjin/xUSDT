...
export class X402SetupClient {
  ...
  setupAgent(config: AgentSetupConfig): AgentSetupResult {
    ...
    const middlewareConfig = this.generateMiddlewareConfig(walletAddress, config.services);
   ...
   */
  private generateMiddlewareConfig(address: Address, services: AgentSetupConfig['services']): string {
...
`.trim();
  ...
  private generateNextJSGuide(): SetupGuide {
    return {
      framework: 'nextjs',
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'npm install @plasma-pay/agent @plasma-pay/x402-setup' },
        { order: 2, title: 'Create middleware', description: 'Add X402 middleware to your API routes' },
        { order: 3, title: 'Configure environment', description: 'Set up environment variables' },
        { order: 4, title: 'Test payment flow', description: 'Verify payments work end-to-end' },
      ],
      codeSnippets: [
        {
...
`.trim(),
      ...
      envVars: [
        { name: 'PLASMA_WALLET_ADDRESS', description: 'Your wallet address to receive payments', required: true, example: '0x...' },
        { name: 'PLASMA_PRIVATE_KEY', description: 'Private key for signing (keep secret!)', required: true, example: '0x...' },
      ],
    };
  }
  ...
  private generateExpressGuide(): SetupGuide {
    return {
      ...
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'npm install @plasma-pay/agent @plasma-pay/x402-setup' },
        { order: 2, title: 'Add middleware', description: 'Use X402 middleware in your Express app' },
        { order: 3, title: 'Configure routes', description: 'Mark routes that require payment' },
      ],
      codeSnippets: [
        {
          filename: 'server.js',
...
`.trim(),
      ...
      envVars: [
        { name: 'PLASMA_WALLET_ADDRESS', description: 'Your wallet address', required: true, example: '0x...' },
      ],
    };
  }
  ...
  private generateFastifyGuide(): SetupGuide {
    return {
      ...
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'npm install @plasma-pay/agent @plasma-pay/x402-setup' },
        { order: 2, title: 'Register plugin', description: 'Add X402 plugin to Fastify' },
      ],
      codeSnippets: [
        {
          filename: 'server.ts',
...
`.trim(),
  ...
  private generateHonoGuide(): SetupGuide {
    return {
      ...
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'npm install @plasma-pay/agent @plasma-pay/x402-setup' },
        { order: 2, title: 'Add middleware', description: 'Use X402 middleware in Hono' },
      ],
      codeSnippets: [
        {
          filename: 'index.ts',
...
`.trim(),
  ...
  private generateFlaskGuide(): SetupGuide {
    return {
      ...
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'pip install plasma-pay' },
        { order: 2, title: 'Add decorator', description: 'Use @require_payment decorator' },
      ],
      codeSnippets: [
        {
          filename: 'app.py',
...
`.trim(),
  ...
  private generateFastAPIGuide(): SetupGuide {
    return {
      ...
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'pip install plasma-pay' },
        { order: 2, title: 'Add dependency', description: 'Use X402 dependency injection' },
      ],
      codeSnippets: [
        {
          filename: 'main.py',
...
`.trim(),
  ...
  private generateVercelEdgeGuide(): SetupGuide {
    return {
      ...
      steps: [
        { order: 1, title: 'Install dependencies', description: 'Add Plasma Pay SDK', command: 'npm install @plasma-pay/x402-setup' },
        { order: 2, title: 'Create edge function', description: 'Add X402 to edge middleware' },
      ],
      codeSnippets: [
        {
...
`.trim(),
      ...
      envVars: [
        { name: 'PLASMA_WALLET_ADDRESS', description: 'Your wallet address (add to wrangler.toml)', required: true, example: '0x...' },
      ],
    };
  }
}
