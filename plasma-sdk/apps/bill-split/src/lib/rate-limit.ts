 ...
 * - /api/bills: 10 requests/minute per wallet
 */
...
/**
 * Rate limiter for /api/scan-receipt endpoint.
 * 5 requests per minute per IP - protects against OpenAI API costs.
 */
export const scanReceiptLimiter = createRateLimiter({
  maxRequests: 5,
  windowMs: 60 * 1000, // 1 minute
});

/**
 * Rate limiter for /api/bills endpoint.
 * 10 requests per minute per wallet address.
 */
export const billsLimiter = createRateLimiter({
  maxRequests: 10,
  windowMs: 60 * 1000, // 1 minute
});
...
if (typeof setInterval !== 'undefined') {
  setInterval(() => {
    cleanupRateLimiter(scanReceiptLimiter);
    cleanupRateLimiter(billsLimiter);
