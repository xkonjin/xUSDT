...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Demo mode: Streams are not persisted and no real funds are transferred.',
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    // Validate addresses
    if (!sender.match(/^0x[a-fA-F0-9]{40}$/) || !recipient.match(/^0x[a-fA-F0-9]{40}$/)) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      demoMode: true,
      message: 'Demo mode: No actual funds were transferred. In production, this would lock USDT0 in a streaming contract.',
