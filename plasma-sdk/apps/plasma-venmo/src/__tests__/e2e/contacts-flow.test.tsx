...
function ContactsTestComponent({
  ...
  const handleToggleFavorite = async (contactId: string, isFavorite: boolean) => {
    // Optimistic update
    setContacts(contacts.map(c => (c.id === contactId ? { ...c, isFavorite } : c)));
    ...
    try {
      ...
      if (!res.ok) {
        // Revert
        setContacts(contacts.map(c => (c.id === contactId ? { ...c, isFavorite: !isFavorite } : c)));
  ...
  return (
    <div>
      ...
      {/* Add/Edit Form */}
      <button onClick={() => setShowAddForm(true)} data-testid="add-contact-button">
        Add Contact
      </button>
      ...
      {showAddForm && (
        <div data-testid="contact-form">
          ...
          <button onClick={resetForm} data-testid="cancel-form">Cancel</button>
          <button
      ...
      {favoriteContacts.length > 0 && !searchQuery && (
        <div data-testid="favorite-contacts">
          ...
          {favoriteContacts.map(contact => (
            <div key={`fav-${contact.id}`} data-testid={`favorite-${contact.id}`}>
              {contact.name}
            </div>
          ))}
        </div>
      ...
      {filteredContacts.length === 0 ? (
        searchQuery ? (
          <div data-testid="no-results">No contacts found for "{searchQuery}"</div>
        ) : (
          <div data-testid="empty-state">
            <p>No contacts yet</p>
            <button onClick={() => setShowAddForm(true)} data-testid="add-first-contact">
        ...
        <div data-testid="contacts-list">
          {filteredContacts.map(contact => (
            ...
            >
              ...
              {contact.isFavorite && <span data-testid={`favorite-icon-${contact.id}`}>⭐</span>}
...
describe('Contacts Flow E2E Tests', () => {
  ...
  describe('Add Contact', () => {
    it('should show add contact form when clicking add button', async () => {
      ...
      const contacts = [createMockContact({ id: '1', name: 'Existing Contact' })];
    ...
    it('should add contact with all fields', async () => {
      const user = userEvent.setup();
      const existingContact = createMockContact({ id: 'existing', name: 'Existing' });
      ...
      await user.type(screen.getByTestId('address-input'), TEST_ADDRESSES.RECIPIENT_ADDRESS);
      await user.click(screen.getByTestId('save-contact'));

      await waitFor(() => {
        expect(screen.getByTestId('contact-new-contact')).toBeInTheDocument();
        expect(screen.getByTestId('contact-name-new-contact')).toHaveTextContent('Alice Smith');
      });
    });
    ...
    it('should show error when API fails', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('error-message')).toHaveTextContent('Duplicate contact');
      });
    });
  ...
  describe('Edit Contact', () => {
    it('should populate form with existing contact data', async () => {
      ...
      expect(screen.getByTestId('email-input')).toHaveValue('original@email.com');
    ...
    it('should update contact name', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('contact-name-edit-name')).toHaveTextContent('New Name');
      });
    });
    ...
    it('should update contact email and phone', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('contact-email-edit-details')).toHaveTextContent('new@email.com');
        expect(screen.getByTestId('contact-phone-edit-details')).toHaveTextContent('+2222222222');
      });
    });
  ...
  describe('Delete Contact', () => {
    it('should delete contact after confirmation', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('contact-delete-me')).not.toBeInTheDocument();
      });
    });
    ...
    it('should revert on delete failure', async () => {
      const user = userEvent.setup();
      const contact = createMockContact({ id: 'fail-delete', name: 'Will Fail' });
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('error-message')).toHaveTextContent(/delete/i);
      });
    });
  });
  ...
  describe('Search Contacts', () => {
    ...
    it('should filter contacts by wallet address', async () => {
      const user = userEvent.setup();
      const contacts = [
        createMockContact({ id: '1', name: 'Alice', contactAddress: TEST_ADDRESSES.VALID_ADDRESS }),
        createMockContact({ id: '2', name: 'Bob', contactAddress: TEST_ADDRESSES.RECIPIENT_ADDRESS }),
      ];
  ...
  describe('Recent Recipients', () => {
    it('should show recent contacts section', () => {
      const contacts = [
        createMockContact({ id: '1', name: 'Recent Alice', lastPayment: '2025-01-25T00:00:00Z' }),
        createMockContact({ id: '2', name: 'Recent Bob', lastPayment: '2025-01-24T00:00:00Z' }),
      ];
    ...
    it('should order by most recent payment', () => {
      const contacts = [
        createMockContact({ id: 'older', name: 'Older', lastPayment: '2025-01-20T00:00:00Z' }),
        createMockContact({ id: 'newer', name: 'Newer', lastPayment: '2025-01-25T00:00:00Z' }),
      ];
    ...
    it('should not show contacts without lastPayment in recent', () => {
      const contacts = [
        createMockContact({ id: '1', name: 'Has Payment', lastPayment: '2025-01-25T00:00:00Z' }),
    ...
    it('should select contact when clicking recent', async () => {
      ...
      const contact = createMockContact({ id: 'recent-select', name: 'Select Me', lastPayment: '2025-01-25T00:00:00Z' });

      render(<ContactsTestComponent initialContacts={[contact]} onSelectContact={onSelect} />);

      await user.click(screen.getByTestId('recent-recent-select'));

      expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ id: 'recent-select' }));
    });

    it('should hide recent section when searching', async () => {
      const user = userEvent.setup();
      const contact = createMockContact({ id: '1', name: 'Test', lastPayment: '2025-01-25T00:00:00Z' });
  ...
  describe('Favorites', () => {
    it('should show favorite icon for favorited contacts', () => {
      const contacts = [
        ...
        createMockContact({ id: '2', name: 'Not Favorited', isFavorite: false }),
      ];
    ...
    it('should toggle favorite on click', async () => {
      const user = userEvent.setup();
      const contact = createMockContact({ id: 'toggle-fav', name: 'Toggle Me', isFavorite: false });

      global.fetch = jest.fn().mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ contact: { ...contact, isFavorite: true } }),
      });

      render(<ContactsTestComponent initialContacts={[contact]} />);

      expect(screen.queryByTestId('favorite-icon-toggle-fav')).not.toBeInTheDocument();

      await user.click(screen.getByTestId('toggle-favorite-toggle-fav'));

      await waitFor(() => {
        expect(screen.getByTestId('favorite-icon-toggle-fav')).toBeInTheDocument();
      });
    });

    it('should remove favorite on click', async () => {
      const user = userEvent.setup();
      const contact = createMockContact({ id: 'unfav', name: 'Unfavorite Me', isFavorite: true });

      global.fetch = jest.fn().mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ contact: { ...contact, isFavorite: false } }),
      });
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('favorite-icon-unfav')).not.toBeInTheDocument();
      });
    });
    ...
    it('should revert favorite on API error', async () => {
      const user = userEvent.setup();
      const contact = createMockContact({ id: 'fail-fav', name: 'Fail', isFavorite: false });
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('favorite-icon-fail-fav')).not.toBeInTheDocument();
  ...
  describe('Contact Selection', () => {
    it('should call onSelectContact when clicking contact', async () => {
      ...
      expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ id: 'select-me', name: 'Click Me' }));
    });
  });
});
