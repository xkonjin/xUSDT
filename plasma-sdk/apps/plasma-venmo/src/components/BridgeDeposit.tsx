...
import { useAllWallets, useConnectExternalWallet } from "@plasma-pay/privy-auth";
...
export function BridgeDepositModal({ recipientAddress, onClose, onSuccess }: BridgeDepositProps) {
  ...
  const [quoteSecondsLeft, setQuoteSecondsLeft] = useState<number>(QUOTE_VALIDITY_SECONDS);
  ...
  const [txState, setTxState] = useState<{
    status: "idle" | "connecting" | "approving" | "bridging" | "waiting" | "success" | "error";
    txHash?: string;
    error?: string;
  }>({ status: "idle" });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const elapsed = Math.floor((Date.now() - quote.lastUpdated!.getTime()) / 1000);
  ...
  useEffect(() => {
    async function fetchBalance() {
      ...
      try {
        ...
        setSourceBalance({ loading: false, balance: null, error: 'Failed to fetch balance' });
      }
    }
    ...
    try {
        ...
        if ((switchError as { code?: number })?.code === 4902) {
          await provider.request({
            ...
            params: [{
              ...
              blockExplorerUrls: selectedChain.explorerUrl ? [selectedChain.explorerUrl] : [],
            }],
          });
        } else {
          throw switchError;
      ...
      if (txData.transaction?.approval) {
        ...
        const approveTx = await provider.request({
          ...
          params: [{
            ...
            to: approval.token,
            data: `0x095ea7b3000000000000000000000000${approval.spender.slice(2)}${BigInt(approval.amount).toString(16).padStart(64, '0')}`,
          }],
        });
      ...
      const txHash = await provider.request({
        ...
        params: [{
          ...
          ...(txData.transaction.gasLimit ? { gas: `0x${parseInt(txData.transaction.gasLimit).toString(16)}` } : {}),
        }],
      }) as string;
  ...
  async function waitForTransaction(provider: unknown, txHash: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const checkReceipt = async () => {
        try {
          const receipt = await (provider as { request: (args: { method: string; params: unknown[] }) => Promise<unknown> }).request({
  ...
  return (
    <ModalPortal isOpen={true} onClose={onClose || (() => undefined)} zIndex={120}>
      <div 
        ref={modalRef}
        className="relative w-full max-w-md bg-gradient-to-br from-white/[0.12] to-white/[0.06] backdrop-blur-xl border border-white/15 rounded-3xl p-6 max-h-[90vh] overflow-y-auto"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[rgb(0,212,255)] to-purple-500 flex items-center justify-center">
              <ArrowRightLeft className="w-5 h-5 text-white" aria-hidden="true" />
            </div>
            <div>
              <h3 id="bridge-modal-title" className="text-xl font-bold text-white">Bridge to USDT0</h3>
        ...
        {hasExternalWallet && externalWallets[0] && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3 mb-4 flex items-center gap-2">
            ...
            <span className="text-green-400 text-sm">
              Connected: {externalWallets[0].address.slice(0, 6)}...{externalWallets[0].address.slice(-4)}
            </span>
          </div>
        )}
        ...
        <div className="bg-white/5 rounded-2xl p-4 mb-3">
          <div className="flex items-center justify-between mb-3">
            <span className="text-white/50 text-sm" id="from-label">From</span>
            {/* Chain Selector */}
            <div className="relative">
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 text-white/50 transition-transform ${showChainPicker ? 'rotate-180' : ''}`} aria-hidden="true" />
              </button>
                ...
                >
                  {POPULAR_SOURCE_CHAINS.map((chain) => (
                    ...
                    >
                      ...
                      {chain.chainId === selectedChain.chainId && (
                        <Check className="w-4 h-4 ml-auto text-[rgb(0,212,255)]" aria-hidden="true" />
                      )}
                    </button>
                  ))}
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                aria-label={`Select token, currently ${selectedToken?.symbol || 'none'}`}
              >
                {selectedToken ? (
                  <>
                    <span className="text-white font-bold">{selectedToken.symbol}</span>
                    <ChevronDown className={`w-4 h-4 text-white/50 transition-transform ${showTokenPicker ? 'rotate-180' : ''}`} aria-hidden="true" />
                  </>
                ) : (
                ...
                >
                  {availableTokens.map((token) => (
                    ...
                    >
                      ...
                      <span className="text-white/40 text-sm truncate">{token.name}</span>
                      {token.address === selectedToken?.address && (
                        <Check className="w-4 h-4 ml-auto text-[rgb(0,212,255)]" aria-hidden="true" />
                      )}
                    </button>
          ...
          {hasExternalWallet && selectedToken && (
            <div id="balance-display" className="flex items-center justify-between mt-3 pt-3 border-t border-white/10">
              <span className="text-white/50 text-xs">Your Balance</span>
              <span className={`text-xs ${!hasSufficientBalance() && amount ? 'text-red-400' : 'text-white/70'}`}>
                {sourceBalance.loading ? (
                  <Loader2 className="w-3 h-3 animate-spin inline" aria-label="Loading balance" />
                ) : sourceBalance.balance ? (
                  <>
                    {sourceBalance.balance} {selectedToken.symbol}
                    {!hasSufficientBalance() && amount && (
                      <AlertTriangle className="w-3 h-3 inline ml-1 text-red-400" aria-hidden="true" />
                    )}
                  </>
                ) : (
                  '--'
        ...
        <div className="flex justify-center -my-1 relative z-10">
          <div className="w-10 h-10 rounded-full bg-gray-900 border-4 border-[#1a1a2e] flex items-center justify-center">
            <ChevronDown className="w-5 h-5 text-[rgb(0,212,255)]" aria-hidden="true" />
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-2xl font-bold text-white">
              {quote.loading ? (
                <span className="flex items-center gap-2">
                  <Loader2 className="w-5 h-5 animate-spin" aria-label="Loading quote" />
            ...
            {quote.best && (
              <button
                ...
                title="Refresh quote"
                aria-label={quote.loading ? "Refreshing quote" : "Refresh quote"}
              >
                <RefreshCw className={`w-4 h-4 text-white/50 ${quote.loading ? "animate-spin" : ""}`} aria-hidden="true" />
              </button>
            )}
          </div>
        </div>
        ...
        {quote.best && (
          <div className="bg-white/5 rounded-2xl p-4 mb-4 space-y-3">
            ...
            {quoteSecondsLeft <= QUOTE_WARNING_THRESHOLD && quoteSecondsLeft > 0 && (
              <div className="flex items-center gap-2 text-amber-400 text-xs pb-2 border-b border-white/10">
                <AlertTriangle className="w-4 h-4" aria-hidden="true" />
                <span>Quote expires in {quoteSecondsLeft}s - prices may change</span>
              </div>
            )}
            ...
            {quoteSecondsLeft === 0 && (
              <div className="flex items-center justify-between text-amber-400 text-xs pb-2 border-b border-white/10">
                ...
                <button
                  onClick={fetchQuote}
                  disabled={quote.loading}
                  className="text-[rgb(0,212,255)] hover:underline disabled:opacity-50"
                >
                  Refresh
                </button>
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-lg" aria-hidden="true">{PROVIDER_LOGOS[quote.best.provider]}</span>
            ...
            {/* All Quotes */}
            {showAllQuotes && (
              <div className="space-y-2 pt-2 border-t border-white/10" role="list" aria-label="All available routes">
                {quote.all.map((q, idx) => (
                  <div
                    key={`${q.provider}-${idx}`}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span aria-hidden="true">{PROVIDER_LOGOS[q.provider]}</span>
        ...
        {amount && !hasSufficientBalance() && sourceBalance.balance && (
          <div className="bg-amber-500/10 rounded-xl p-3 mb-4" role="alert">
            <div className="flex items-start gap-2 text-amber-400 text-sm">
              ...
              <span>
                Insufficient balance. You have {sourceBalance.balance} {selectedToken?.symbol} but need {amount}.
              </span>
            </div>
          </div>
        )}
