...
describe("TransactionHistory", () => {
  ...
  it("shows loading skeleton initially", () => {
    ...
    // Should show skeleton during loading (uses TransactionListSkeleton)
    expect(screen.getByText("Recent Activity")).toBeInTheDocument();
    expect(document.querySelector(".bg-white\\/3")).toBeInTheDocument();
  });
  ...
  it("shows empty state when no transactions", async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({ transactions: [], total: 0, hasMore: false }),
    });
  ...
  it("shows transaction count", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/showing 2 of 2 transactions/i)
      ).toBeInTheDocument();
    });
  });
