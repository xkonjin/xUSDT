...
type PaymentMethod = 'venmo' | 'revolut' | 'wise' | 'cashapp' | 'zelle' | 'paypal';
...
export function ZKP2POnrampV2({
  ...
  const [step, setStep] = useState<'amount' | 'method' | 'redirect'>('amount');
  ...
  const handleOpenZKP2P = useCallback(() => {
  ...
  }, [recipientAddress, amount, currency, selectedMethod, onSuccess]);
  ...
  const selectedPaymentOption = PAYMENT_OPTIONS.find(p => p.id === selectedMethod);
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-sm">Instant, trustless on-ramp</p>
            </div>
          </div>
          {onClose && (
            <button
      ...
      <div className="px-6 py-3 border-b border-white/5">
        <div className="flex items-center gap-2">
          {['Amount', 'Payment', 'Complete'].map((label, index) => {
            const stepIndex = ['amount', 'method', 'redirect'].indexOf(step);
              ...
              <div key={label} className="flex items-center flex-1">
                <div className="flex items-center gap-2">
                  <div
                    ...
                    } ${isCurrent ? 'ring-2 ring-plenmo-500/50 ring-offset-2 ring-offset-transparent' : ''}`}
                  >
                ...
                {index < 2 && (
                  <div className={`flex-1 h-0.5 mx-2 rounded ${isActive && index < stepIndex ? 'bg-plenmo-500' : 'bg-white/10'}`} />
                )}
              </div>
            );
          })}
          ...
          {step === 'method' && (
            ...
            >
              ...
              <div className="space-y-2 mb-4">
                {PAYMENT_OPTIONS.filter(p => p.popular && p.currencies.includes(currency)).map((payment) => (
                  <button
                    key={payment.id}
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-white/50 text-sm">Pay with {payment.name}</p>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {PAYMENT_OPTIONS.filter(p => !p.popular && p.currencies.includes(currency)).map((payment) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-white text-sm">{payment.name}</span>
                  </button>
                ))}
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="mb-6">
                ...
                <p className="text-white/50">
                  {CURRENCY_SYMBOLS[currency]}{amount} {currency} → USDC
                </p>
              </div>
