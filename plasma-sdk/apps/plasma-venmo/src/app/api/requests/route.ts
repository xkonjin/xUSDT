...
export async function POST(request: Request) {
  ...
  try {
    ...
    const addressValidation = validateAddress(fromAddress);
    if (!addressValidation.valid) {
      return NextResponse.json(
        { error: addressValidation.error || "Invalid fromAddress" },
        { status: 400 }
      );
    }
    ...
    if (isEmail) {
      const notification = await notifyHelpers.create({
        ...
        body: `${
          fromEmail || fromAddress.slice(0, 6) + "..."
        } requested $${amount} USDT0`,
        data: {
      ...
      try {
        ...
        if (!notifyResponse.ok) {
          console.error(
            "[requests] Failed to send notification email:",
