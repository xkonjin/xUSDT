#!/usr/bin/env python3
"""
GLM-4.7 Sequential Swarm Orchestrator for xUSDT/Plenmo Analysis
Runs specialized agents sequentially to avoid rate limits
"""
...
def call_glm_agent(agent: Dict, context: str) -> Dict:
...
def main():
    print("=" * 60)
    print("GLM-4.7 Sequential Swarm Orchestrator")
    print("=" * 60)
    ...
    print(f"\n[2/3] Running {len(AGENTS)} specialized agents sequentially...")
    results = []
    
    for i, agent in enumerate(AGENTS):
        print(f"\n  [{i+1}/{len(AGENTS)}] Running {agent['role']}...")
        result = call_glm_agent(agent, context)
        results.append(result)
        print(f"    Status: {result['status']}")
        
        # Wait between calls to avoid rate limiting
        if i < len(AGENTS) - 1:
            print("    Waiting 3 seconds before next agent...")
            time.sleep(3)
    
    successful = sum(1 for r in results if r['status'] == 'success')
    print(f"\nCompleted: {successful}/{len(results)} agents successful")
